...
/*
function preload() {
  this.load.image('playerSprite', 'pictures/BluePlanet.png');
...
function preload() {
    // Fetch the player's selected sprite
    fetch('/api/playerSprite')
        .then(response => response.json())
        .then(data => {
            // Extract the file name from the API response
            const playerSpriteFileName = data.fileName ;
            this.load.image('playerSprite', `pictures/${playerSpriteFileName}`);
        })
        .catch(error => {
            console.error('Error fetching player sprite file name:', error);
